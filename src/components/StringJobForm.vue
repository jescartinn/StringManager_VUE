...
<template>
    <div class="string-job-form">
        <v-container class="string-job-form__container">
            ...
            <v-row>
                <v-col cols="12">
                    <h1 class="string-job-form__title">
                        {{ mode === 'create' ? 'Create New String Job' : 'Edit String Job' }}
                    </h1>
                </v-col>
            </v-row>
            ...
            <!-- Loading Skeleton -->
            <template v-if="loadingReferences || (mode === 'edit' && loading)">
                <v-row>
                    <v-col cols="12">
            ...
            <!-- Form Content -->
            <v-form v-else @submit.prevent="handleSubmit" v-model="formValid" class="string-job-form__form">
                <v-card class="mb-4">
                        ...
                        <v-row>
                            ...
                            <v-col cols="12" md="6">
                                <v-select v-model="formData.racquetId" :items="playerRacquets.map(r => ({
                                    ...
                                    :error-messages="errors.racquetId" :disabled="!playerSelected || mode === 'edit'"
                                    :loading="racquetStore.loading" variant="outlined" required>
                                    <template v-slot:prepend>
                                        <v-icon>mdi-tennis</v-icon>
                                    </template>
                                    <template v-slot:append-inner>
                                        <v-btn v-if="playerSelected && mode === 'create'" icon="mdi-plus" size="small"
                                            variant="text" color="primary" title="Add New Racquet"
                ...
                <v-card class="mb-4">
                    ...
                    <v-card-text>
                        <v-row>
                            ...
                            <v-col cols="12" md="6">
                                <v-select v-model="formData.crossStringId" :items="stringTypeStore.stringTypeOptions"
                                    item-title="text" item-value="value" label="Cross String (if different)"
                                    variant="outlined" :loading="stringTypeStore.loading" clearable
                                    :disabled="!formData.mainStringId">
                                    <template v-slot:prepend>
                                        <v-icon>mdi-grid</v-icon>
                                    </template>
                                </v-select>
                            ...
                            <v-col cols="12" sm="6" md="3">
                                <div class="d-flex">
                                    ...
                                    <v-btn icon variant="text" color="primary" class="ml-2 mt-2"
                                        ...
                                        <v-icon>{{ usesCrossTension ? 'mdi-link' : 'mdi-link-variant-off' }}</v-icon>
                                    </v-btn>
                                </div>
                            </v-col>
                ...
                <v-card class="mb-4">
                    ...
                    <v-card-text>
                        <v-row>
                            ...
                            <!-- Notes Text Area -->
                            <v-col cols="12">
                                <v-textarea v-model="formData.notes" label="Notes" variant="outlined" rows="3" clearable
                ...
                <!-- Form Actions -->
                <div class="d-flex justify-end gap-4 mb-8">
